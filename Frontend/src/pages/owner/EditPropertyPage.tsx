...
import {
  ArrowLeft, ArrowRight, Check, Home, DollarSign,
  MapPin, Camera, Save, Building2
} from 'lucide-react';
...
const mockProperty = {
  ...
  description: 'Experience luxury living at its finest in this stunning penthouse overlooking the Manhattan skyline. Floor-to-ceiling windows provide breathtaking views of the city that never sleeps.',
  category: 'Penthouse',
...
const amenityOptions = [
  ...
  'Balcony', 'Garden', 'Elevator', 'Pet Friendly',
];
...
export function EditPropertyPage() {
  ...
  const [selectedAmenities, setSelectedAmenities] = useState<string[]>(['WiFi', 'Pool', 'Gym', 'TV']);
  const [images, setImages] = useState<string[]>(['from-purple-500 to-pink-500', 'from-blue-500 to-cyan-500']);
  const [isLoading, setIsLoading] = useState(false);

  const { register, handleSubmit, formState: { errors }, trigger, watch } = useForm<EditPropertyForm>({
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="font-heading text-4xl text-secondary">Edit Property</h1>
            <p className="text-gray-500 font-ui text-sm mt-1">{mockProperty.name}</p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <AnimatePresence mode="wait">
              {step === 0 && (
                <motion.div key="s0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input type="text" {...register('name')} className={`w-full px-4 py-3 border rounded-xl text-sm font-ui focus:outline-none focus:border-primary transition-colors ${errors.name ? 'border-error' : 'border-gray-200'}`} />
                  ...
                  <div>
                    ...
                    <textarea {...register('description')} rows={4} className={`w-full px-4 py-3 border rounded-xl text-sm font-ui focus:outline-none focus:border-primary transition-colors resize-none ${errors.description ? 'border-error' : 'border-gray-200'}`} />
                  ...
                  <div>
                    ...
                    <select {...register('category')} className={`w-full px-4 py-3 border rounded-xl text-sm font-ui focus:outline-none focus:border-primary transition-colors ${errors.category ? 'border-error' : 'border-gray-200'} bg-white`}>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" {...register('address')} className={`w-full pl-10 pr-4 py-3 border rounded-xl text-sm font-ui focus:outline-none focus:border-primary transition-colors ${errors.address ? 'border-error' : 'border-gray-200'}`} />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" {...register('city')} className={`w-full px-4 py-3 border rounded-xl text-sm font-ui focus:outline-none focus:border-primary transition-colors ${errors.city ? 'border-error' : 'border-gray-200'}`} />
                    ...
                    <div>
                      ...
                      <input type="text" {...register('country')} className={`w-full px-4 py-3 border rounded-xl text-sm font-ui focus:outline-none focus:border-primary transition-colors ${errors.country ? 'border-error' : 'border-gray-200'}`} />
              ...
              {step === 3 && (
                <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    {images.length < 8 && (
                      <button type="button" onClick={addMockImage} className="h-40 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center gap-2 hover:border-primary hover:bg-accent/50 transition-colors">
              ...
              {step === 4 && (
                <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  <h2 className="font-ui font-semibold text-xl text-secondary mb-4">Review Changes</h2>
            ...
            {step < 4 && (
              <div className="flex gap-3 mt-6 pt-6 border-t border-gray-100">
                {step > 0 && (
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="button" onClick={prevStep} className="flex-1 border border-gray-200 text-secondary font-ui font-semibold py-3 rounded-xl hover:border-primary transition-colors flex items-center justify-center gap-2">
                    <ArrowLeft className="w-4 h-4" /> Back
                  </motion.button>
                )}
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="button" onClick={nextStep} className={`${step === 0 ? 'w-full' : 'flex-1'} bg-primary hover:bg-primary-dark text-white font-ui font-semibold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors`}>
